...
class PCPart	{
	...
	public function decrementInventory()	{ $this->pcPartInventory -= 1; }
}
...
class Controller	{
	...
	public function getPCPartFromBuildType($buildId, $typeInt)	{
		
		$orderedItems = $this->getOrderedItemsByCustomBuildId($buildId);
		$pcParts = null;
		foreach($orderedItems as $orderedItem)	{
			$pcPart = $this->getPCPartById($orderedItem->pcPartId);
			
			if($pcPart->getPcPartType() == $typeInt)	{
				$pcParts = $pcPart;
			}
		}
	...
	public function getPCPartsByCompatibilityAndType($compatibilityInt, $partType)	{
		...
		$query;
		if($compatibilityInt == 0)	{
			$query = "SELECT * FROM PCParts WHERE pcPartType = '$partType'";
		}
	...
	public function getBuildsFeatured()	{
		
		$query = "SELECT * FROM customBuilds WHERE buildFeatured = 1";
        $result = $this->con->query($query);
        if($result->num_rows > 0){
	...
	public function getBuildsAccount($accountId)	{
		
		$query = "SELECT * FROM customBuilds WHERE accountId = $accountId";
        $result = $this->con->query($query);
        if($result->num_rows > 0){
	...
	public function getPCPartsByCustomBuildIdAndType($buildId, $partType)	{
		...
		foreach($orderedItems as $orderedItem)	{
			...
			if($pcPart->getPcPartType() == $partType)	{
				$pcParts[] = $pcPart;
			}
		}
	...
	public function getBuildsOrdered()	{
		
		$query = "SELECT * FROM customBuilds WHERE buildOrdered = 1";
        $result = $this->con->query($query);
        if($result->num_rows > 0){
	...
	public function getMostOrderedParts()	{
		
		$query = "SELECT * FROM PCParts ORDER BY pcPartOrderedTimes LIMIT 5";
        $result = $this->con->query($query);
        if($result->num_rows > 0){
	...
	public function getPCPartsLimit($limit, $pageNumber)	{
		
		$offset = ($pageNumber - 1)*$limit;
		
		$query = "SELECT * FROM PCParts LIMIT ".$offset.", $limit";
	...
	public function getBuildsOrderedLimit($limit, $pageNumber)	{
		
		$offset = ($pageNumber - 1)*$limit;
		
		$query = "SELECT * FROM customBuilds WHERE buildOrdered = 1 LIMIT ".$offset.", $limit";
        $result = $this->con->query($query);
	...
	public function getBuildsAccountLimit($limit, $pageNumber, $accountId)	{
		
		$offset = ($pageNumber - 1)*$limit;
		
		$query = "SELECT * FROM customBuilds WHERE accountId = $accountId LIMIT ".$offset.", $limit";
        $result = $this->con->query($query);
	...
	public function getPCPartsByCompatibilityAndTypeLimit($limit, $pageNumber, $compatibilityInt, $partType)	{
		
		$query;
		$offset = ($pageNumber - 1)*$limit;
		if($compatibilityInt == 0)	{
			$query = "SELECT * FROM PCParts WHERE pcPartType = '$partType' LIMIT ".$offset.", $limit";
		}
		
		else	{
			$query = "SELECT * FROM PCParts WHERE pcPartCompatibility = $compatibilityInt AND pcPartType = '$partType' LIMIT ".$offset.", $limit";
		}
	...
	public function getPCPartsBySearchCompatibilityAndTypeLimit($limit, $pageNumber, $inputSearch, $compatibilityInt, $partType)	{
		...
		if($compatibilityInt == 0)	{
			...
			$query = "SELECT * FROM PCParts WHERE (pcPartName LIKE '%"."$inputSearch"."%' OR pcPartDescription LIKE '%"."$inputSearch"."%') AND pcPartType = '$partType' LIMIT ".$offset.", $limit";
		}
		
		else	{
			
			$query = "SELECT * FROM PCParts WHERE (pcPartName LIKE '%"."$inputSearch"."%' OR pcPartDescription LIKE '%"."$inputSearch"."%') AND pcPartType = '$partType' AND pcPartCompatibility = $compatibilityInt LIMIT ".$offset.", $limit";
		}
	...
	public function getPCPartsBySearchCompatibilityAndType($inputSearch, $compatibilityInt, $partType)	{
		
		$query;
		
		if($compatibilityInt == 0)	{
			
			$query = "SELECT * FROM PCParts WHERE (pcPartName LIKE '%"."$inputSearch"."%' OR pcPartDescription LIKE '%"."$inputSearch"."%') AND pcPartType = '$partType'";
		}
		
		else	{
			
			$query = "SELECT * FROM PCParts WHERE (pcPartName LIKE '%"."$inputSearch"."%' OR pcPartDescription LIKE '%"."$inputSearch"."%') AND pcPartType = '$partType' AND pcPartCompatibility = $compatibilityInt";
		}
	...
	public function insertCustomBuildByTemplate($accountObj, $customBuildTemplateObj)	{
		...
		$accountId = $this->con->real_escape_string($accountObj->getId());
		$buildName = $this->con->real_escape_string('copy of '.$customBuildTemplateObj->buildName);
		
		$query1 = "INSERT INTO customBuilds (accountId, buildName, buildCreationTime, buildOrdered, buildFeatured, buildComment) VALUES ($accountId, '$buildName', CURDATE(), 0, 0, '')";
		...
		if($sql == true){
			...
			$query2 = "SELECT MAX(customBuildId) as max FROM customBuilds";
			$buildId = $this->con->query($query2);
			if($buildId->num_rows > 0){
				$orderedItemsTemplate = $this->getOrderedItemsByCustomBuildId($customBuildTemplateObj->getId());
				
					while($row = $buildId->fetch_assoc())	{
						foreach($orderedItemsTemplate as $orderedItemObj)	{
						$query3 = "INSERT INTO orderedItems (customBuildId, pcPartId) VALUES (".$row['max'].", ".$orderedItemObj->pcPartId.")";
	...
	public function insertOrderedItemsUNUSED($PCPartObj, $customBuildObj)	{
		
		$customBuildId = $this->con->real_escape_string($customBuildObj->id);
		$pcPartId = $this->con->real_escape_string($PCPartObj->id);
	...
	public function insertCustomBuild($accountObj)	{
		...
		$query = "INSERT INTO customBuilds (accountId, buildName, buildCreationTime, buildOrdered, buildFeatured, buildComment) VALUES ($accountId, '$buildName', CURDATE(), 0, 0, '')";
		$sql = $this->con->query($query);
	...
	public function insertOrderedItems($OrderedItemObj)	{
		
		$customBuildId = $this->con->real_escape_string($OrderedItemObj->getCustomBuildId());
		$pcPartId = $this->con->real_escape_string($OrderedItemObj->getPcPartId());
	...
	public function deleteOrderedItemByPartId($partId)	{
		
		$query = "DELETE FROM orderedItems WHERE pcPartId = $partId";
		$sql = $this->con->query($query);
	...
	public function startAndGetBuild($accountObj)	{
		
		$buildId = null;
		$this->insertCustomBuild($accountObj);
		
		$query = "SELECT MAX(customBuildId) as max FROM customBuilds";
		$result = $this->con->query($query);
		...
		return $this->getBuildFromId($buildId);
	}
	public function startAndGetBuildFromTemplate($accountObj, $templateBuildId)	{
		
		$buildId = null;
		$this->insertCustomBuildByTemplate($accountObj, $this->getBuildFromId($templateBuildId));
	...
	public function getPartTypes()	{
		...
		return array("cpu", "motherboard", "memory", "storage", "video Card", "case");
	}
	public function getPresentPartTypesFromBuild($buildId)	{
		
		$partTypes = $this->getPartTypes();
		$pcParts = $this->getPCPartsByCustomBuildId($buildId);
		
		$presentPartTypes = Array();
		
		foreach($pcParts as $pcPart)	{
			$i = 0;
			foreach($partTypes as $partType)	{
				
				if($pcPart->getPcPartType() == $i)	{
		...
		return $presentPartTypes;
	}

	public function getIndexCuts($indexNumber, $limit, $itemCount, $indexNumLimit)	{
		
		$numbers = Array();
		$pageCuttoff = ceil($itemCount/$limit);
		
		$lowestIndex = $indexNumber - floor($indexNumLimit / 2);
		$highestIndex = $indexNumber + floor($indexNumLimit / 2);
			
		if($highestIndex > $pageCuttoff)	{
				
			$highestIndex = $pageCuttoff;
		}
	...
	public function getCompatibilityFromBuild($buildId)	{
		
		$partsList = $this->getPCPartsByCustomBuildId($buildId);
		$compatibility = 0;
		
		foreach($partsList as $pcPart)	{
			
			if($pcPart->getPcPartCompatibility() > $compatibility)	{
				$compatibility = $pcPart->getPcPartCompatibility();
			}
		}
	...
	public function replacePartBuildIfTypeExistAddIfNot($buildId, $partId)	{
		
		$pcPart = $this->getPCPartById($partId);
		$type = $pcPart->getPcPartType();
		
		foreach($this->getPCPartsByCustomBuildId($buildId) as $pcPartt)	{
			
			if($type == $pcPartt->getPcPartType())	{
				
				$this->deleteOrderedItemByPartId($pcPartt->getId());
			}
		}
		
		$orderedItem = new OrderedItem(0, $buildId, $partId);
		
		$this->insertOrderedItems($orderedItem);
	}

	public function getCompatibilityFromBuildAsideFrom($buildId, $typeInt)	{
